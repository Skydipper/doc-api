...
```shell
curl -X GET https://api.skydipper.com/v1/graph/query/list-concepts
```
...
```shell
curl -X GET https://api.skydipper.com/v1/graph/query/concepts-inferred?concepts=<concept_list>
```
...
```url
https://api.skydipper.com/v1/graph/query/concepts-inferred?concepts=spain,raster
```
...
```shell
https://api.skydipper.com/v1/graph/query/concepts-ancestors?concepts=<concept_list>
```
...
```url
https://api.skydipper.com/v1/graph/query/concepts-ancestors?concepts=forest_cover,landslide
```
...
```shell
curl -X GET https://api.skydipper.com/v1/graph/query/similar-dataset/<dataset-id>
```
...
```url
https://api.skydipper.com/v1/graph/query/similar-dataset/63a7a997-695d-4629-b6e9-9b169f5c69bf?published=true&env=production,preproduction&app=skydipper&limit=6
```
...
```shell
curl -X GET https://api.skydipper.com/v1/graph/query/similar-dataset-including-descendent/<dataset-id>
```
...
```url
https://api.skydipper.com/v1/graph/query/similar-dataset-including-descendent/03bfb30e-829f-4299-bab9-b2be1b66b5d4?published=true&env=production,preproduction&app=skydipper&limit=6
```
...
```shell
curl -X GET https://api.skydipper.com/v1/graph/query/search-datasets?concepts[0][0]='water'
```
...
`https://api.skydipper.com/v1/graph/query/search-datasets?concepts[0][0]=spain,concepts[0][1]=europe,concepts[1][0]=water,concepts[2][0]=raster,concepts[2][1]=geospatial`
...
Search for datasets with the tag `global` and `water` and at least one of the two tags: `raster`, `geospatial`. The resulting datasets should also be published and be categorized either as `production` or `preproduction` and belong to the application `skydipper`. The total number of results returned won't be limited since a very high number has been provided as limit _(999999)_.

`https://api.skydipper.com/v1/graph/query/search-datasets?concepts[0][0]=global&concepts[1][0]=water&concepts[2][0]=raster&concepts[2][1]=geospatial&published=true&env=production,preproduction&app=skydipper&page[size]=999999`
...
```shell
curl -X GET https://api.skydipper.com/v1/graph/query/most-liked-datasets
```
